...
All rights reserved. Â© 2026 HexaStack Solutions.
...
## Local Docker (from former DEPLOYMENT_GUIDE.md)

A `docker-compose.yml` at the repo root can launch the app stack
locally. Use **pnpm** (not npm) to match `packageManager` and CI.
...
Database schema changes use Drizzle only:

```bash
pnpm run db:push
```

Do not use Prisma â€” the unused `prisma/` folder was removed in the
repo cleanup. Live schema: `drizzle/schema.ts`.
